...
public class NexusDownloadManager implements DownloadListener {
    ...
    private Runnable getTask(ModuleToInstall module, String mvnUri) {
        ...
        if (serverBean != null) {
            return new CustomServerDownloaderRunnable(serverBean, module, mvnUri, this);
        }
        return new NexusDownloaderRunnable(module, mvnUri, this);
    }
...
class CustomServerDownloaderRunnable extends AbsDownLoaderRunnable {

    private ArtifactRepositoryBean customNexusServer;

    CustomServerDownloaderRunnable(ArtifactRepositoryBean customNexusServer, ModuleToInstall module, String url,
            NexusDownloadManager downloadManager) {
        super(module, url, downloadManager);
        this.customNexusServer = customNexusServer;
    }

    protected void doDownLoad() throws Exception {
        downloadManager.downloadStart(-1);
        ILibraryManagerService libManager = (ILibraryManagerService) GlobalServiceRegister.getDefault()
                .getService(ILibraryManagerService.class);
        libManager.resolveJar(customNexusServer, url);
